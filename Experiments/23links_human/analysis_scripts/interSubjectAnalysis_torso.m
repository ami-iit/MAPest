...
% Plot mean
fig = figure('Name', 'jC1Head roty','NumberTitle','off');
axes1 = axes('Parent',fig,'FontSize',16);
box(axes1,'on');
hold(axes1,'on');
grid on;
for blockIdx = 1 : block.nrOfBlocks
    subplot (5,1,blockIdx)
    % NE
    plot1 = plot(singleJointsTau.torso.jointsPerArea(blockIdx).C1Head_roty.torqueMeanNE,'color',orangeAnDycolor,'lineWidth',4);
    axis tight;
    ax = gca;
    ax.FontSize = 20;
    hold on
    % WE
    plot2 = plot(singleJointsTau.torso.jointsPerArea(blockIdx).C1Head_roty.torqueMeanWE,'color',greenAnDycolor,'lineWidth',4);
    hold on
    title(sprintf('Block %s', num2str(blockIdx)),'FontSize',22);
    ylabel('$\bar\tau$ [Nm]','HorizontalAlignment','center',...
    'FontSize',30,'interpreter','latex');
