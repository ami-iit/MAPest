...
for linAccIdx = 1  : nrOfLinAccelerometer
    covTun.total_variables_meas = [covTun.total_variables_meas; covTun.data_power3.data(linAccIdx).meas];
end
% Filling with the 6D wrenches
tmp.fextIndex = []; % Define range in data (only for block 1) for forces
for fextInDataIdx = 1 : length(data)
    if data(fextInDataIdx).type == 1002
...
for vectOrderIdx = 1 : length(dVectorOrder)
    for dataFextIdx = tmp.fextIndex(1) : tmp.fextIndex(end)
        if strcmp(covTun.data_power3.data(dataFextIdx).id,dVectorOrder{vectOrderIdx})
            covTun.total_variables_meas = [covTun.total_variables_meas; covTun.data_power3.data(dataFextIdx).meas];
            break;
        end
    end
end
...
% filling with the linear acceleration
for linAccIdx = 1  : nrOfLinAccelerometer
    covTun.total_variables_estim_power1 = [covTun.total_variables_estim_power1; covTun.y_sim_linAcc_power1.y_sim_linAcc.meas{linAccIdx,1}];
    covTun.total_variables_estim_power2 = [covTun.total_variables_estim_power2; covTun.y_sim_linAcc_power2.y_sim_linAcc.meas{linAccIdx,1}];
    covTun.total_variables_estim_power3 = [covTun.total_variables_estim_power3; covTun.y_sim_linAcc_power3.y_sim_linAcc.meas{linAccIdx,1}];
    covTun.total_variables_estim_power4 = [covTun.total_variables_estim_power4; covTun.y_sim_linAcc_power4.y_sim_linAcc.meas{linAccIdx,1}];
end
% filling with the 6D wrenches
for vectOrderIdx = 1 : length(dVectorOrder)
